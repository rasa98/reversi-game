...
class HeuFunctionInterface(ABC):
    def __init__(self, *params):
        self.params = [self.gen_ratio_per_turn()] + list(params)

    @staticmethod
    def gen_ratio_per_turn():
        return np.random.rand(60)  # 60 turns max in game

    def mutate_ratio(self):
        ratio = self.params[0]
        noise = np.random.normal(0, 0.1, ratio.size)

        # Add noise to the original numbers
        mutated_ratio = ratio + noise

        # Use modulo operation to wrap around values that go out of bounds
        mutated_ratio = mutated_ratio % 1
        self.params[0] = mutated_ratio
    ...
    def mutate(self):
        ...
        indexes = range(1, len(self.params))  # skip 0 index, its ratio
        self.mutate_ratio()
        for idx in indexes:
    ...
    def crossover(self, other):
        ...
        for i in param_index_other:
            if i == 0:
                ratio_index_other = random.sample(list(range(60)),
                                                  random.randint(1, 59))
                self.params[0][ratio_index_other] = other.params[0][ratio_index_other]
                continue
