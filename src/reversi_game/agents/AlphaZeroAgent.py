...
from reversi_game.algorithms.alphazero.alpha_mcts import MCTS
...
def load_azero_agent(name, model_location=None, model=None, params=None):
    if model_location is None and model is None:
    ...
    if model is None:
        ...
        # model = load_model(model_location, res_blocks, hidden_layer)
...
def model_generator(model_location, model_idxs, params):
    for i in model_idxs:
        model_location = f'{model_location}/model_{i}.pt'
        try:
