...
def find_normal_neigh(fields, board):
    res = set()
    for field in fields:
        res.update(check_empty_edge_fields(field, board))
    return res


def find_normal_neigh_njit(fields, board):
    res = set()
    for field in fields:
        res.update(njit_check_empty_edge_fields(board, field))
    return res


@njit(cache=True)
def find_normal_neigh_njit_full(fields, board):
    result = []
    for field in fields:
        target_row, target_col = field[0], field[1]
...
def find_normal_neigh_numpy(fields, board):
    fields = np.array(list(fields))
    index_pairs = fields + DIRECTIONS_ARR[:, np.newaxis]
    index_pairs = index_pairs.reshape(-1, index_pairs.shape[2])
    index_pairs = np.unique(index_pairs, axis=0)

    num_rows, num_cols = board.shape
    # Filter out out-of-bounds indices
    valid_mask = (index_pairs[:, 0] < num_rows) & (index_pairs[:, 1] < num_cols)
    valid_pairs = index_pairs[valid_mask]

    rows, cols = valid_pairs[:, 0], valid_pairs[:, 1]
    mask = board[rows, cols] == 0
    filtered_pairs = valid_pairs[mask]
    return filtered_pairs.tolist()


def check_empty_edge_fields(field, board):
...
class Othello:
    ...
    def __init__(self, players=("w", "b"), turn=1, board=None,
                 first_move=1, last_move=None, fields_1=None,
                 fields_2=None,
        ...
        if fields_1 is None and fields_2 is None:
            self.fields_1 = {(3, 3), (4, 4)}
            self.fields_2 = {(3, 4), (4, 3)}
        else:
            self.fields_1 = fields_1
            self.fields_2 = fields_2
    ...
    def get_snapshot(self):
        return Othello(players=(self.white, self.black),
                       ...
                       fields_1=self.fields_1.copy(),
                       fields_2=self.fields_2.copy(),
    ...
    def get_edge_fields(self):
        s = self.fields_2 if self.player_turn == 1 else self.fields_1
        return find_normal_neigh_njit(s, self.board)

    def _calculate_next_valid_moves(self):  # TODO make test to confirm old and new give same results
        moves_to_reverse = {}
        list_of_fields = []
        edge_fields = self.get_edge_fields()
        if len(edge_fields):
            edge_fields = np.array(list(edge_fields))
    ...
    def play_move(self, field):
        if self.winner is None:  # if the game haven't ended, you can play
            ...
            if field in self.valid_moves():
                ...
                self.update_fields_1_2(field, to_reverse)
    ...
    def update_fields_1_2(self, field, to_reverse):
        add = self.fields_2
        remove = self.fields_1
        if self.player_turn == 1:
            add = self.fields_1
            remove = self.fields_2
        add.add(field)
        add.update(to_reverse)
        remove.difference_update(to_reverse)
